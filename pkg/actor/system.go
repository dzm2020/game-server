...
type ISystem interface {
	...
	SpawnActor(handler ActorHandler, opts ...Option) (PID, error)
	Tell(from PID, target any, msg any) error
	Ask(from PID, target any, msg any, timeout time.Duration) (any, error)
...
func NewSystemWithNodeID(nodeID string) *System {
	...
	s := &System{
		processDict: maputil.NewConcurrentMap[PID, *process](0),
...
type System struct {
	processDict *maputil.ConcurrentMap[PID, *process]
...
func (s *System) Spawn(handler Handler, opts ...Option) (PID, error) {
	...
	return s.SpawnActor(messageHandlerAdapter{fn: handler}, opts...)
}

func (s *System) SpawnActor(handler ActorHandler, opts ...Option) (PID, error) {
	if handler == nil {
		return NoSender, ErrNilHandler
	}
...
func (s *System) addProcess(proc *process) error {
	...
	s.processDict.GetOrSet(proc.getPID(), proc)
	return nil
}
...
func (s *System) removeProcess(proc *process) {
	...
	s.processDict.Delete(proc.getPID())
...
func (s *System) Tell(from PID, target any, msg any) error {
	if s.closed.Load() {
...
func (s *System) Ask(from PID, target any, msg any, timeout time.Duration) (any, error) {
	if s.closed.Load() {
