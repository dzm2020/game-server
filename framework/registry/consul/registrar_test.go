...
func newIntegrationRegistrar(t *testing.T) (*Registrar, *api.Client) {
	...
	return newRegistrar(client), client
}
...
func testTTLConfig(ttl, interval time.Duration, note string, deregisterAfter time.Duration) Config {
	cfg := Config{
		TTL:               ttl,
...
func TestRegistrarRegisterHeartbeatAndDeregister(t *testing.T) {
	...
	err := rr.Register(ServiceInstance{
		ID:                serviceID,
		Name:              serviceName,
		Address:           "127.0.0.1",
		Port:              8080,
	}, testTTLConfig(5*time.Second, 300*time.Millisecond, "hb", 5*time.Second))
	if err != nil {
...
func TestRegistrarRegisterValidation(t *testing.T) {
	rr := newRegistrar(nil)
	err := rr.Register(ServiceInstance{}, Config{})
	if err == nil {
...
func TestRegistrarUpdateTTLRealConsul(t *testing.T) {
	...
	if err := rr.Register(ServiceInstance{
		ID:                serviceID,
		Name:              serviceName,
		Address:           "127.0.0.1",
		Port:              8088,
	}, testTTLConfig(5*time.Second, 400*time.Millisecond, "init", 0)); err != nil {
		t.Fatalf("register failed: %v", err)
	}
